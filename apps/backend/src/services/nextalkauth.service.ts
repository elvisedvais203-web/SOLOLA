...
import { createTwoFactorChallenge, resolveTwoFactorChallenge } from "./nextalkaccount-security.service";
...
export async function loginWithEmailPassword(input: { email: string; password: string; ipAddress?: string; userAgent?: string }) {
  ...
  if (user.twoFactorEnabled && user.twoFactorSecret) {
    ...
    return {
      ...
      user: { id: user.id, email: user.email, planTier: user.planTier, role: user.role }
    };
  }
...
export async function completeTwoFactorLogin(input: {
  ...
  const user = await resolveTwoFactorChallenge(input.challengeId, input.token);
